...
namespace Prototype.Editor
{
    public class NestedAssetsWindow : EditorWindow
    {
        ...
        static readonly Dictionary<Type, (Type type, string extension)> rules = new()
        {
            {
                /* asset */
        ...
        static bool CheckRule(System.Type assetType, System.Type childAssetType)
        {
            foreach (var key in rules.Keys.Where(x => assetType == x || assetType.IsSubclassOf(x)))
                if (childAssetType == rules[key].type || childAssetType.IsSubclassOf(rules[key].type))
                    return true;
            return false;
        }
        ...
        void Refresh()
        {
            ...
            nestedObjectsList.ToListElement(
                ...
                deleteItem: item =>
                {
                    ...
                    string ext = null;
                    foreach (var (type, extension) in rules.Values)
                    {
                        if (itemType != type)
                            continue;
